...
@Component({
    ...
    providers: [EventService, UserService, UploadService, EmailService]
})
export class EventAddComponent {
    ...
    ngDoCheck(){
        ...
        //console.log(this.status_general);
    }
    ...
    submit(){
        ...
        this.event.date = moment(this.selectedDate,['DD-MM-YYYY']).format('YYYY-MM-DD') + " " + this.hour+':00.000Z';
        console.log(this.event);

        let token = this._userService.getToken();
        console.log(token);

        this._eventService.addEvent(this.event, token).subscribe(
            response => {
                if(!response.event){
                    console.log('error');
                }else{
                    ...
                    /* send email to interested users on the event */
                    this._emailService.sendMail(eventCreated, token).subscribe(
                        response => {
                            if(response.state == 'success'){ 
                                console.log('Correos avisando sobre el evento, enviados...');
                            }else{
                                console.log('Error al enviar emails');
                            }
                        },
                        error => {
            ...
            error => {
                console.log(error);
                if(error.status == 404){
                    this.status_general = 'repeated-event';
